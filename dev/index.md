...
[`later::run_now()`](https://later.r-lib.org/dev/reference/run_now.md).
...
return the relevant file descriptors to be monitored.

## Usage from C++

You can also call
[`later::later`](https://later.r-lib.org/dev/reference/later.md) from
C++ code in your own packages, to cause your own C-style functions to be
called back. This is safe to call from either the main R thread or a
different thread; in both cases, your callback will be invoked from the
main R thread.

[`later::later`](https://later.r-lib.org/dev/reference/later.md) is
...
[`later::later_fd`](https://later.r-lib.org/dev/reference/later_fd.md)
is also accessible from `later_api.h` and its prototype looks like this:
...
the first being an `int*` array provided by
[`later_fd()`](https://later.r-lib.org/dev/reference/later_fd.md) when
called back, and the second being a `void*`. The `int*` array will be
the length of `num_fds` and contain the values `0`, `1` or `NA_INTEGER`
to indicate the readiness of each file descriptor, or an error condition
